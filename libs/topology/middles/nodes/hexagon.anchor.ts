...
export function hexagonAnchors(node: Node) {
  node.anchors.push(new Point(node.rect.x + node.rect.width / 5, node.rect.y, Direction.Up));
  node.anchors.push(new Point(node.rect.x + (node.rect.width * 4) / 5, node.rect.y, Direction.Up));
